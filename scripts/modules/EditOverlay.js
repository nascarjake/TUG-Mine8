	...
	app.controller('EditOverlayCtrl', ['$scope', 'msgHan', function($scope, msgHan) {
		...
		$scope.resize = function(){
			if($elm != null){
	            ...
	            } catch (e) {console.log('EditOverlay.resize error')}
	            $('.edit').show();
		...
		$scope.hideDetailSection = function(){
			msgHan.fire('AreYouSurePopup.open', {message: "Are you sure you want to hide the built-in details section? You can unhide it in the top bar later.", cb: function() {
		...
		$scope.displayContentItem = function(item){
			var items = contentTypes.filter(function(e){return e.value == item.Type})
			if(items.length > 0 )
		...
		$scope.openMediaBrowser = function(target){
			msgHan.fire('MediaBrowser.open', {filter: target, cb: function(media){
