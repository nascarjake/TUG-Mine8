		...
		return {
			get: function(cb){
				...
				$http.get(url).then(function(msg){
					...
					msgHan.fire('notify', {message: "Couldn't Load Mod Manifest.", type: "error"})
				})
			},
			fileInfo: function(username, password, cb){
				...
				$http.get(url).then(function(msg){
					...
					console.log(err)
					msgHan.fire('deepRefresh');
					msgHan.fire('notify', {message: "Couldn't retrieve Git repo information. Please try again later.", type: "error"})
				})
			},
			save: function(username, password, content, sha, cb){
				...
				$http.put(url, data).then(function(msg){
					...
					msgHan.fire('AlertPopup.open', {title: "Mod Saved", message:"Your manifest has been updated. Changes may take a few minutes to take effect."})
					if(cb)cb()
				}, function(err){
					console.log(err)
					msgHan.fire('notify', {message: "Couldn't retrieve Git repo information. Please try again later.", type: "error"})
